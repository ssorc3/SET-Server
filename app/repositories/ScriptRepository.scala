...
class ScriptRepository @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) extends HasDatabaseConfigProvider[JdbcProfile] {
  ...
  def deleteUserScript(userID: String, scriptName: String): Future[Any] = db.run{
    scripts.filter(s => s.userID === userID && s.scriptName === scriptName).delete
  }
  ...
  def getUserLastRun(userID: String, scriptName: String): Future[Seq[Long]] = db.run {
    scripts.filter(s => s.userID === userID && s.scriptName === scriptName).map(_.lastRun).result
  }
}