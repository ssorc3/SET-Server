...
class DeviceRepository @Inject()(protected val dbConfigProvider:DatabaseConfigProvider)(implicit ec: ExecutionContext) extends HasDatabaseConfigProvider[JdbcProfile]{
  ...
  def getAllDevices(): Future[Seq[Device]] = db.run {
    devices.result
  }

  def getDeviceZone(deviceID: String): Future[Seq[Int]] = db.run {
    devices.filter(_.deviceID === deviceID).map(_.zoneID).result
  }
